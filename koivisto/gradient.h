    ...
    void  init(Board* b, EvalData* ev) {
        phase =
            (24.0f + phaseValues[5]
             ...
             - phaseValues[1]
                   * bitCount(b->getPieceBB()[WHITE_KNIGHT] | b->getPieceBB()[BLACK_KNIGHT])
             - phaseValues[2]
                   * bitCount(b->getPieceBB()[WHITE_BISHOP] | b->getPieceBB()[BLACK_BISHOP])
...
struct Pst64Data {
    // to quickly recompute the values for the piece square tables, we require the indices of each
    // square and how often is being used. We also need to know if we deal with same side or opposite
    // side castling
    ...
    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        for (Piece p = PAWN; p < KING; p++) {
            for (int i = 1; i <= indices_white[p][0]; i++) {
                int8_t w = indices_white[p][i];
                td->w_piece_square_table[p][!sameside_castle][w].midgame.gradient += mg_grad;
                td->w_piece_square_table[p][!sameside_castle][w].endgame.gradient += eg_grad;
            }
            for (int i = 1; i <= indices_black[p][0]; i++) {
                int8_t b = indices_black[p][i];
                td->w_piece_square_table[p][!sameside_castle][b].midgame.gradient -= mg_grad;
                td->w_piece_square_table[p][!sameside_castle][b].endgame.gradient -= eg_grad;
            }
        }
        ...
        for (int i = 1; i <= indices_white[KING][0]; i++) {
            int8_t w = indices_white[KING][i];
            td->w_piece_square_table[KING][0][w].midgame.gradient += mg_grad;
            td->w_piece_square_table[KING][0][w].endgame.gradient += eg_grad;
        }
        for (int i = 1; i <= indices_black[KING][0]; i++) {
            int8_t b = indices_black[KING][i];
            td->w_piece_square_table[KING][0][b].midgame.gradient -= mg_grad;
            td->w_piece_square_table[KING][0][b].endgame.gradient -= eg_grad;
        }
    }
};
...
struct Pst225Data {
        ...
        for (Piece p : {PAWN, QUEEN}) {
            for (int c = 0; c <= 1; c++) {
                ...
                while (k) {
                    ...
                    if (c == WHITE) {
                        indices_white_wk[index][++indices_white_wk[index][0]] =
                            (pst_index_relative_white(s, wKingSq));
                        indices_white_bk[index][++indices_white_bk[index][0]] =
                            (pst_index_relative_white(s, bKingSq));
                    } else {
                        indices_black_wk[index][++indices_black_wk[index][0]] =
                            (pst_index_relative_black(s, wKingSq));
                        indices_black_bk[index][++indices_black_bk[index][0]] =
                            (pst_index_relative_black(s, bKingSq));
                    }
    ...
    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        ...
        for (Piece p : {PAWN, QUEEN}) {
            for (int i = 1; i <= indices_white_wk[index][0]; i++) {
                uint8_t w = indices_white_wk[index][i];
                td->w_piece_our_king_square_table[p][w].midgame.gradient += mg_grad;
                td->w_piece_our_king_square_table[p][w].endgame.gradient += eg_grad;
            }

            for (int i = 1; i <= indices_white_bk[index][0]; i++) {
                uint8_t w = indices_white_bk[index][i];
                td->w_piece_opp_king_square_table[p][w].midgame.gradient += mg_grad;
                td->w_piece_opp_king_square_table[p][w].endgame.gradient += eg_grad;
            }
            for (int i = 1; i <= indices_black_bk[index][0]; i++) {
                uint8_t b = indices_black_bk[index][i];
                td->w_piece_our_king_square_table[p][b].midgame.gradient -= mg_grad;
                td->w_piece_our_king_square_table[p][b].endgame.gradient -= eg_grad;
            }

            for (int i = 1; i <= indices_black_wk[index][0]; i++) {
                uint8_t b = indices_black_wk[index][i];
                td->w_piece_opp_king_square_table[p][b].midgame.gradient -= mg_grad;
                td->w_piece_opp_king_square_table[p][b].endgame.gradient -= eg_grad;
            }
            index++;
        }
    }
            ...
            for (int c = 0; c <= 1; c++) {
                ...
                while (k) {
                    ...
                    switch (p) {
                        ...
                        case BISHOP:
                            attacks = lookUpBishopAttack(square, occupied & ~b->getPieceBB(c, QUEEN));
                            break;
                        case QUEEN:
...
struct BishopPawnTableData {
    ...
    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        for (int i = 0; i < 9; i++) {
            td->w_bishop_pawn_e[i].midgame.gradient += count_e[i] * mg_grad;
            td->w_bishop_pawn_e[i].endgame.gradient += count_e[i] * eg_grad;
            //            midgame += count_e[i] * w_bishop_pawn_e[i].midgame.value;
            //            endgame += count_e[i] * w_bishop_pawn_e[i].endgame.value;

            td->w_bishop_pawn_o[i].midgame.gradient += count_o[i] * mg_grad;
            td->w_bishop_pawn_o[i].endgame.gradient += count_o[i] * eg_grad;
        ...
        for (Color color : {WHITE, BLACK}) {
            ...
            while (bb) {
                ...
                if (!passed && (sqBB & ev->semiOpen[color])) {
                    ...
                    U64 pawnAdvance =
                        color == WHITE ? shiftNorth(sqBB) : shiftSouth(sqBB);    // advance square
                    U64 levers =
                        oppPawns
                        & (color == WHITE
                                 ?    // levers are pawns in active tension
                    ...
                    U64 forwardLevers =
                        oppPawns
                        & (color == WHITE ?    //
                               (shiftNorthEast(pawnAdvance) | shiftNorthWest(pawnAdvance))
                                            :    // levers that would apply if pawn was advanced
                               (shiftSouthEast(pawnAdvance) | shiftSouthWest(pawnAdvance)));    //
                    U64 helpers = (shiftEast(sqBB) | shiftWest(sqBB))
                                  & pawns;    // friendly pawns on either side

                    bool push = !(antiPassers ^ levers);    // Are all the pawns currently levers, we
                                                            // can make this pawn passed by pushing it
                    bool helped =
                        !(antiPassers ^ forwardLevers)    // Are all the pawns forward lever, we can
    ...
    void evaluate(float& midgame, float& endgame, ThreadData* td) {
        for (int i = 0; i < 8; i++) {
            midgame += count[i] * td->w_passer[i].midgame.value
                       + candidate[i] * td->w_candidate[i].midgame.value;
            endgame += count[i] * td->w_passer[i].endgame.value
                       + candidate[i] * td->w_candidate[i].endgame.value;
        }
    }
...
struct FeatureData {
    ...
    void   init(Board* b, EvalData* ev) {
        ...
        // all doubled pawns
        U64    whiteDoubledPawns =
            whiteDoubledWithoutFirst | (wRearSpans(whiteDoubledWithoutFirst) & whitePawns);
        U64 blackDoubledPawns =
            blackDoubledWithoutFirst | (bRearSpans(blackDoubledWithoutFirst) & blackPawns);

        // all isolated pawns
        U64 whiteIsolatedPawns =
            whitePawns & ~(fillFile(shiftWest(whitePawns) | shiftEast(whitePawns)));
        U64 blackIsolatedPawns =
            blackPawns & ~(fillFile(shiftWest(blackPawns) | shiftEast(blackPawns)));
        ...
        for (Color color : {WHITE, BLACK}) {
            ...
            while (bb) {
                ...
                if (!(passerMask & oppPawns)) {
                    ...
                    count[I_PAWN_PASSED_AND_BLOCKED] +=
                        bitCount(teleBB & b->getTeamOccupiedBB(!color)) * h;
                    ...
                    count[I_PAWN_PASSED_HELPER] +=
                        (bitCount(
                            pawns
                            & (color == WHITE ? wAttackRearSpans(pawns) : bAttackRearSpans(pawns))))
                        * h;

                    // check if its defended
                    count[I_PAWN_PASSED_AND_DEFENDED] +=
                        (bitCount(sqBB & ev->pawnWestAttacks[color])
                         + bitCount(sqBB & ev->pawnEastAttacks[color]))
                        * h;

                    // check if can be caught by king
                    count[I_PAWN_PASSED_SQUARE_RULE] +=
                        ((7 - r + (color != b->getActivePlayer())) < manhattanDistance(
                             bitscanForward(promBB), bitscanForward(b->getPieceBB(!color, KING))))
                        * h;

                    count[I_PAWN_PASSED_KING_TROPISM] +=
                        h
                        * std::clamp(chebyshevDistance(oppKingSq, adv)
                                         - chebyshevDistance(kingSq, adv),
                                     -4, 4);
                }
    ...
    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        ...
        for (int i = 0; i < I_END; i++) {
            td->w_features[i].midgame.gradient += count[i] * mg_grad;
    ...
    void   init(Board* b, EvalData* ev) {
        ...
        for (int color = 0; color <= 1; color++) {
            ...
            while (potentialPinners) {
                ...
                // if there is exactly one of our pieces in the way, consider it pinned. Otherwise,
                // continue
                ...
                // add to the result indexing using pinnedPiece for which there are 5 different pieces
                // and the pinner
...
struct HangingData {
    ...
    void   init(Board* b, EvalData* ev) {
        ...
        for (int i = PAWN; i <= QUEEN; i++) {
            count[i] = +bitCount(b->getPieceBB(WHITE, i) & WnotAttacked)
    ...
    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        ...
        for (int i = 0; i < 5; i++) {
            td->w_hanging[i].midgame.gradient += count[i] * mg_grad;
...
struct MobilityData {
    ...
    void    init(Board* b, EvalData* ev) {
        ...
        U64    whitePawnCover =
            shiftNorthEast(b->getPieceBB(WHITE, PAWN)) | shiftNorthWest(b->getPieceBB(WHITE, PAWN));
        U64 blackPawnCover =
            shiftSouthEast(b->getPieceBB(BLACK, PAWN)) | shiftSouthWest(b->getPieceBB(BLACK, PAWN));
        ...
        for (Piece p = KNIGHT; p <= QUEEN; p++) {
            for (int c = 0; c <= 1; c++) {
                ...
                while (k) {
                    ...
                    if (indices_white[p][0] >= 4 && c == WHITE
                        || indices_black[p][0] >= 4 && c == BLACK) {
                        std::cout << "cannot have mobility entries with more than 4 pieces per side"
                                  << std::endl;
                        std::exit(-1);
                    }

                    if (c == WHITE) {
                        indices_white[p][++indices_white[p][0]] =
                            (bitCount(attacks & mobilitySquaresWhite));
                    } else {
                        indices_black[p][++indices_black[p][0]] =
                            (bitCount(attacks & mobilitySquaresBlack));
                    }
...
struct PosEvalData {
    ...
    void gradient(float lossgrad, int threadID) {
        ...
        float mg_grad = 0;
        float eg_grad = 0;
        meta.gradient(mg_grad, eg_grad, lossgrad, &threadData[threadID]);

        features.gradient(mg_grad, eg_grad, &threadData[threadID]);
        mobility.gradient(mg_grad, eg_grad, &threadData[threadID]);
        ...
        pst225.gradient(mg_grad, eg_grad, &threadData[threadID]);
    }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int n = 0; n < 2; n++) {
                for (int j = 0; j < 64; j++) {
                    ...
                    threadData[t].w_piece_square_table[i][n][j] = {{w1}, {w2}};
                }
            }
            ...
            if (i < 5) {
                ...
                for (int n = 0; n < 15 * 15; n++) {
                    threadData[t].w_piece_opp_king_square_table[i][n].set(
                        piece_opp_king_square_table[i][n]);

                    threadData[t].w_piece_our_king_square_table[i][n].set(
                        piece_our_king_square_table[i][n]);
                }
...
void merge_gradients() {
    for (int t = 1; t < N_THREAD; t++) {
        for (int i = 0; i < 6; i++) {
            ...
            for (int n = 0; n < 2; n++) {
                for (int j = 0; j < 64; j++) {
                    threadData[t].w_piece_square_table[i][n][j].merge(
                        threadData[0].w_piece_square_table[i][n][j]);
                }
            }
            ...
            if (i < 5) {
                ...
                for (int n = 0; n < 15 * 15; n++) {
                    threadData[t].w_piece_opp_king_square_table[i][n].merge(
                        threadData[0].w_piece_opp_king_square_table[i][n]);
                    threadData[t].w_piece_our_king_square_table[i][n].merge(
                        threadData[0].w_piece_our_king_square_table[i][n]);
                }
...
void share_weights() {
    for (int t = 1; t < N_THREAD; t++) {
        for (int i = 0; i < 6; i++) {
            ...
            for (int n = 0; n < 2; n++) {
                for (int j = 0; j < 64; j++) {
                    threadData[t].w_piece_square_table[i][n][j].set(
                        threadData[0].w_piece_square_table[i][n][j]);
                }
            }
...
void train(int iterations, float K, float eta) {
    for (int i = 0; i < iterations; i++) {
        startMeasure();
        std::cout << left;
        std::cout << "loss= " << setw(20) << compute_loss(K) << " eps= " << setw(20)
                  << positions.size() / stopMeasure() * 1000 << std::endl;
        adjust_weights(eta);
    }
}
...
float compute_K(double initK, double rate, double deviation) {
    ...
    while (abs(dEdK) > deviation) {
        ...
        std::cout << "K:" << K << " Error: " << (Epdk + Emdk) / 2 << " dev: " << abs(dEdK)
                  << std::endl;
